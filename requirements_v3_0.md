...
Updated in v3.0.1 to include `PermissionsTransferAnyEntity`, `PermissionsTransferAnyLead`, `PermissionsTransferAnyCase` so the Transfer Record permission and its legacy variants are present in the dataset and selectable in the Prescribe Access picker (see §2.72 BUG-16). Updated again in v3.8 (§2.95 DATA-2) to add `PermissionsQueryAllFiles` plus 72 other governance-relevant system permissions (data access/query-scope, user/access admin, identity/session security, privacy/compliance, metadata/packaging/deploy), bringing the column set to 80 and matching Query 13's column set exactly. The full BUG-15 mandate ("every `Permissions*` boolean on Profile" — all ~835 in the target org) remains a partially-applied target; the remaining ~755 columns are narrow feature-specific toggles (Tableau, Einstein/Agentforce, Slack, CDP, ITSM, etc.) with no identified governance relevance and are out of scope per §2.95.

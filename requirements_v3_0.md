...
### 2.87 UX-64 — Interactive Paired-Diff Explorer (filters + running list) — IMPLEMENTED in v3.2

> **Status:** Implemented in v3.2 (2026-05-29). Requested by Manoj: turn the static Has/Missing/Shared/Differs legend into interactive filter toggles, add a list view with name/category/status badge, and a running list on the right for tracking rows to work on. Scope decisions confirmed up front: lands on **all** paired diffs (not just Mergable); an **AND/OR toggle** controls multi-filter combination; running list is **session-only**.

**Changes (implemented in v3.2):**
- `PairedDiffPane` (the single shared component behind every paired diff — Compare pane, Reconcile, `ConsolidatePairPane` used by the Mergable and Duplicate tabs) rewritten to embed its own controls, so the feature lands everywhere at once.
- **Filter legend → toggle buttons.** The four legend terms (Has / Missing / Shared / Differs) render as pill toggle buttons; each row's status set is derived relative to the pair (`rowStatusSet`): both-sides → `["Shared"]`, exclusive-to-a-side → `["Has","Missing"]`. `Differs` is retained for legend parity but isn't produced by the current per-token CRUD model, so it matches nothing. Descriptions surface as button tooltips.
- **AND/OR combine toggle.** With no filters selected, all rows show. OR shows rows whose status set intersects the selection; AND requires the selection to be a subset of the row's status set (e.g. Has+Missing → exclusive rows; Has+Shared → empty, as expected).
- **Running list sidebar (session-only).** A third column right of the A|B panes. Each row has a `+` button (shows `✓` once added); the list shows a category badge (Object/Field/System via `tokenCategory` token-prefix parse) + label with per-item remove (`×`) and a clear-list action. Held in component `useState`, resets per pair — not persisted to IDB.
- Removed the now-redundant standalone `<PairedDiffLegend>` immediately preceding the two `PairedDiffPane` call sites; the standalone legend is retained at the two table-only sites (Duplicate detail, Reconcile token grid).
- Verified with `esbuild` JSX compile. JSX header, `appVersion` (×3), `package.json`, and `index.html` bumped to v3.2.

---

### 2.88 REFACTOR-1 — Internal cleanup pass (no behavior change) — IMPLEMENTED in v3.3

> **Status:** Implemented in v3.3 (2026-05-29). Requested by Manoj after a run of patches ("a fresh rebuild might be in order"). Scope confirmed: in-place cleanup of the single-file artifact (the runtime requires one file — `index.html` strips the lone import + default export and evals — so no module split). No user-facing behavior change; focus on removing duplication, a stale-state race, and per-pair recomputation.

**Changes (implemented in v3.3):**
- **Dedup sync/async compute paths.** The three sync `precompute*` functions (`precomputeProfileSimilarity`, `precomputePermSetOverlap`, `precomputePermSetOverlapV26`) were near-identical copies of their async generators. They now delegate to the generators via a new `drainGen()` helper, removing ~90 lines of duplicated O(n²) loop bodies and guaranteeing the sync and async paths can't drift.
- **Signature reuse in `subsetExtras`.** Added an optional `sigMap` (id → signature Set) parameter; the Redundancy-tab memo builds it once from `overlap.entries` and passes it in, skipping two `grantorSignature()` walks per subset pair. Falls back to on-demand computation when no map is supplied.
- **Cancellation guards on lazy precomputes.** The deferred `setTimeout` computes in `RedundancyTab`, `MergablePermSetsTab`, and `DuplicatePermSetsTab` now capture a `cancelled` flag and clear pending timers in the effect cleanup, so a tab switch or `idx` change can't overwrite fresh state with a stale result.
- **`setIsSubset()` helper + single-pass intersection.** Replaces the `[...sig].every(...)` / `[...sig].filter(...)` idioms in the overlap loops; short-circuits on the first miss and computes the intersection count in one pass instead of three array spreads. Verified equivalent to the old idioms across 200k randomized set pairs.
- **Centralized version.** New `APP_VERSION` constant is the single source for the pebundle `appVersion` field (was hardcoded in three places — the drift that caused the v3.2 tag/commit mismatch).
- Deleted the obsolete `pe-filter-tracker.jsx` prototype (folded into the app in v3.2). Verified with esbuild compile. Header, `package.json`, `index.html` bumped to v3.3.

**Out of scope (deferred):** async-chunking `precomputeImpactMatrix` (largest surface area — better as its own focused change); virtualizing the 2,000-row scroll lists.

---

### 2.89 PERF-1 — Impact Matrix chunking + list virtualization — IMPLEMENTED in v3.4

> **Status:** Implemented in v3.4 (2026-05-29). The two items deferred from the v3.3 refactor (§2.88). No user-facing behavior change beyond responsiveness.

**Changes (implemented in v3.4):**
- **Impact Matrix chunking.** `precomputeImpactMatrix` (O(users × assignments × effective-perm walks), the worst main-thread blocker — 5–15s on a large org) was converted to a generator `precomputeImpactMatrixGen` that yields every 50 users. `ScenarioA`'s lazy compute and Phase 1 of "Load All Computations" now run it through `asyncChunked()`, so the progress bar paints and the UI stays interactive. `ScenarioA` also gained a `cancelled`-flag guard (no stale write if the tab unmounts / `idx` changes mid-compute). The sync `precomputeImpactMatrix()` is retained as a thin `drainGen()` wrapper for any non-async caller.
- **List virtualization.** New `VirtualList` component (fixed row height, owns its scroll container, renders only the visible window + overscan). Applied to the `ExplorerTab` item picker, whose rows are uniform height per entry type (34px; 48px for the two-line CustomPermission entry). This removes the prior `slice(0, 2000)` cap and the "showing first 2000" notice — the full filtered set now scrolls with a small constant DOM footprint. Windowing verified to cover every on-screen row with no gaps across the full scroll range.
- **Deferred:** the two `slice(0, 2000)` *table* renders (Impact Matrix table with expandable rows; Prescribe-Access before/after diff table) were left bounded as-is — they have sticky headers and variable-height/expandable rows, so virtualizing them is materially riskier for little additional payoff. Candidate for a later pass if those tables grow.
- Verified with esbuild compile. Header / `APP_VERSION` / `package.json` / `index.html` bumped to v3.4.

---

### 2.90 PERF-2 — Table virtualization — IMPLEMENTED in v3.5

> **Status:** Implemented in v3.5 (2026-05-29). The two table renders deferred in §2.89 (PERF-1). Requested by Manoj ("do everything, don't leave anything behind").

**Changes (implemented in v3.5):**
- **New `VirtualTable` component.** Owns its scroll container and table; renders only the visible main rows plus top/bottom spacer `<tr>`s (so scroll height is correct). Main rows are a fixed `rowHeight` (the caller sets `height` + ellipsis so a row can't grow past it and drift the offset math). Expandable detail rows are variable height and measured on mount via a ref (only a handful are ever open, so the measure set stays tiny); cumulative offsets are rebuilt each render (O(n), fine at the few-thousand-row scale). `tableLayout: fixed` + a `<colgroup>` keep header/body columns aligned as rows scroll in and out. Zebra striping is applied inline by data-row index because CSS `nth-child` striping isn't stable once spacer rows and a moving window are involved.
- **Impact Matrix table (Scenario A).** Re-rendered through `VirtualTable` — main row + measured expandable Lost/Covered detail. `slice(0, 2000)` cap removed; the full filtered/sorted set now scrolls. Text columns (User/Profile/PermSet/Group) ellipsize with a `title` tooltip. Trade-off accepted: fixed column widths + inline zebra mean the per-row hover highlight no longer applies on this table.
- **Prescribe-Access Before/After debug table.** Re-rendered through `VirtualTable` (uniform rows, no expansion). `slice(0, 2000)` cap and the "Truncated to first 2,000" notice removed.
- **Verification.** esbuild compile clean; no `slice(0, 2000)` caps remain in the file. Variable-height windowing unit-tested (top/bottom pads always sum to total height; every on-screen row falls within the rendered window across the full scroll range). Visual aspects (row height, ellipsis, sticky header, zebra) require an in-browser eyeball.

---

### 2.91 UX-65 — Compare Users (side-by-side effective-permission diff) — IMPLEMENTED in v3.6

> **Status:** Implemented in v3.6 (2026-05-29). Requested by Manoj: "compare two users' permissions side by side… what permission sets / objects / fields / read/write/create/delete is different," and confirmed to integrate with Change Impact analysis.

**Changes (implemented in v3.6):**
- **New helpers.** `userEffectiveSignature(idx, userId)` flattens `effectiveObjects/effectiveFields/effectiveSysPerms` into one token Set in the standard vocabulary (`O:obj:C|R|E|D|VA|MA`, `F:field:R|E`, `S:perm`) — so it already accounts for profile fan-out, group membership, and muting permsets, and reconciles exactly with the Impact Matrix engine. `diffUserSources(idx, aId, bId)` diffs the two users' assignment sources (profile + PermSets/groups) into `{ shared, onlyA, onlyB }`.
- **`CompareUsersTab` component + nav tab** ("Compare Users", positioned after Change Impact, gated on data loaded). Two user typeaheads (with a swap button), persisted in AppState (`compareUserA/B`). Summary pills (Jaccard, shared / only-A / only-B counts). An assignment-source diff (three columns: only-A / shared / only-B). A token-level effective-permission diff rendered as three `PairedDiffPane`s grouped by Object / Field / System — each carries the v3.2 Has/Missing/Shared filter toggles + running list. An "Only show differences" toggle (drops Shared rows). CSV export of the full token diff.
- **Cross-links (level 2).** From the Explorer user detail: "Compare as User A/B ↗" preloads the user and switches to the Compare tab. From Compare: "Open {user} in Change Impact ↗" sets the impact user filter and switches to that tab. Consistency (level 1) is automatic — both tabs run on the same effective-permission engine.
- **Simulate-the-difference (level 3).** "Make A match B → simulate ↗" computes the PermSets B has that A lacks (profiles excluded — can't be assigned via PSA), pushes them into Simulation Mode via a new batch callback `onSimulationMutateMany` (single `applyAllMutations` so stacked `addPSA` mutations don't clobber each other), and jumps to Change Impact to show the blast radius. Symmetric "Make B match A" provided. Disabled when there are no assignable differences.
- esbuild compile verified. Header / `APP_VERSION` / `package.json` / `index.html` bumped to v3.6.

**Out of scope / notes:** assigns whole PermSets only (not synthetic per-token grants); profile differences are surfaced in the source diff but not auto-applied (no profile-change mutation type exists).

### 2.92 BUG-8b — Profile-owned PermSets render implicit PS Id instead of Profile name — IMPLEMENTED in v3.6.1

> **Status:** Implemented in v3.6.1 (2026-06-04). Reported by Manoj from the Field detail view: the Permission Set Grants table listed profile-owned grants by their implicit PermissionSet Id (e.g. `00e3g000000HjPAAA0`) rather than the owning Profile name, making the row unusable.

**Root cause.** A profile's object/field/system grants are stored under its auto-generated implicit PermissionSet (`IsOwnedByProfile = true`), whose `Label`/`Name` is just the implicit PS Id. Any provenance view that printed `ps.Label` for these rows therefore showed a raw Id. (Distinct from BUG-11, which resolved *standalone* source Ids in the "Granted by" pills but did not special-case profile-owned PermSets.)

**Changes (implemented in v3.6.1):**
- **Index.** New inverse map `profileByImplicitPsId` (implicit PS.Id → Profile), built from `profileImplicitPsByProfileId` and exported on the index object.
- **New helper `grantorLabel(idx, parentId, fallback)`.** Central resolver: a profile-owned PermSet (matched via `profileByImplicitPsId`, or via `ps.IsOwnedByProfile && ps.ProfileId`) resolves to `"<Profile> (profile)"`; otherwise falls back to the permset label/name/id. The `(profile)` suffix disambiguates from a standalone PermSet that happens to share the name.
- **Field detail (§2.13 / UX-11).** PermSet, Group, and Muting grant tables now show the resolved profile name.
- **Object detail.** "Grantors (Profiles & PermSets)" table resolves the name, and the row click-through now navigates to the Profile (previously a no-op permset lookup for implicit-PS rows).
- **`resolveSourceLabel` (BUG-11 chain).** The shared resolver behind every "Granted by" pill (User detail Effective Objects/Fields, grouped field view, etc.) now resolves profile-owned sources to the profile name.
- esbuild compile verified. `APP_VERSION` / `package.json` bumped to v3.6.1.

**Out of scope / notes:** semantic system-perm source tags ("Profile (direct)" / "Implicit PS") and the audit-trail tables already used profile names and were left unchanged; all PermSet *list* views filter out profile-owned PermSets and were unaffected.

---
### 2.93 UX-66 — CSV exports include raw Salesforce Ids alongside names — IMPLEMENTED in v3.7.0

> **Status:** Implemented in v3.7.0 (2026-06-10). Requested by Manoj 2026-06-04, immediately after BUG-8b (§2.92): the on-screen provenance views are now name-first for readability, but CSV exports carried names only, making them unusable for Data Loader loads, joins, and scripts.

**Behavior.** Every CSV export now includes the raw Salesforce Id columns alongside the resolved display names. Exports only — no on-screen rendering changes. One flat row per record; Id columns are appended after the visible columns.

**Changes (implemented in v3.7.0):**
- **`ExportCSVButton` (central change).** At export time the button scans up to the first 50 rows and appends to the column list any own row keys matching `/Ids?$/i` (e.g. `userId`, `permSetId`, `profileId`, `subsetId`, lowercase `id`) that are not already among the visible columns, in order of first appearance. Any export whose rows already carry Id fields gains the Id columns automatically.
- **Row-construction enrichments** (so the auto-append has Ids to find): Impact Matrix rows add `groupId`; Delete-impact per-user rows add `userId`; Subset Relationship extras rows add `subsetId`/`supersetId`; Field detail grant tables (Profile / PermSet / Group / Muting) add `permSetId` and — for profile-owned implicit PermSets, resolved via `profileByImplicitPsId` (§2.92) — `profileId` + `profileName`, so exports carry BOTH the owning Profile and the implicit PS Id; Object detail "Grantors" rows add `permSetId` + `profileId`; Duplicate PermSets rows add `permSetAId`/`permSetBId`; Migration Profile-pairs rows add `profileAId`/`profileBId`.
- **Covered automatically (no change needed):** reconcile, Read/Edit Loss, Impacted-users tables, Assigned Users, Users in Role, Granting PermSets/Profiles, Orphaned/Empty/Dormant/Single-User lists, Group Member PermSets, Permission Sources, coverage tables, Role footprint — their rows already carried Id-suffixed keys.
- **Intentionally skipped:** user_compare.csv token-diff rows, Prescribe-Access report, and purely computed token/count tables (Effective Objects/Fields, Dead Fields, Dismissals, etc.) — no per-row record Ids exist.
- esbuild compile verified. `APP_VERSION` / `package.json` / `index.html` bumped to v3.7.0.

---
### 2.94 BUG-23 — Objects with FLS but no object-level permission are dropped from object views — IMPLEMENTED in v3.7.1

> **Status:** Fix implemented in `PermissionExplorer.jsx` and esbuild-verified (2026-06-15); targeted for the v3.7.1 patch. Changelog/version-bump release pass pending. Reported by Manoj from the Bullhorn Community Profile (a Guest profile): `OrderItem` shows "No object permissions for this grantor" and cannot be found in the profile's Objects list, even though field-level security on `OrderItem` fields is granted.

**Root cause.** Every object-centric view derives its objects exclusively from the `ObjectPermissions`-backed indexes (`objPermsByParent` / `objPermsBySobj`) and never consults the `FieldPermissions` indexes (`fieldPermsByParent` / `fieldPermsBySobj`). When a grantor holds field-level security on an object but has **no** `ObjectPermissions` row for it, the object is silently dropped from object views: absent from the grantor's Objects list, uncounted, and missing as a grantor in the object reverse-lookup. The field grants still render in the Fields sections (the field index is unaffected), which is why the object appears to be "missing data" while its fields are present. This is a normal Salesforce state — FLS can be set on an object whose object-level CRUD is not granted in the same parent.

Confirming evidence (live org): the Bullhorn Community Profile's implicit, profile-owned PermissionSet `0PS0P000000b5cDWAQ` has three `FieldPermissions` on `OrderItem` (`Description` R/E, `ListPrice` R, `ProductCode` R) and zero `ObjectPermissions` on `OrderItem`. Org-wide the gap is broad: `FieldPermissions` reference 746 distinct objects versus 1,179 for `ObjectPermissions`, so any grantor (profile or permset) holding FLS without a co-located object-permission row is affected.

**Changes (implemented in v3.7.1):**
- **New helper `fieldOnlyObjectRows(objPermRows, fieldRows)`.** Given a grantor's already-collected object-permission and field-permission rows, returns synthetic "field-only" object rows (CRUD all `false`, `fieldOnly: true`, plus a `fieldGrantCount`) for the SObjectTypes that appear only in `FieldPermissions`.
- **Grantor detail (Profile + PermSet) — `ParentDetailSubSections`.** `objRows` now appends `fieldOnlyObjectRows(...)`, so the Objects table for any Profile or PermSet lists FLS-only objects, badged `FLS-only (n)` with dashes across CRUD. Fixes both detail views via the one shared component.
- **User effective view — `effectiveObjects()`.** Adds an equivalent pass over each non-muting source's field permissions, inserting any object not already present as a `fieldOnly` entry with all-false CRUD. The User detail "Effective objects" list and count include them, and they obey the existing UX-23 "hide zero-grant objects" toggle (they read as zero-grant).
- **Object reverse-lookup — `ObjectDetail`.** Unions FLS-only grantors into the "Grantors (Profiles & PermSets)" table and the "Grantors" count, badged `FLS-only (n)`.
- esbuild compile verified (jsx loader, react external) — zero errors/warnings.

**Out of scope / notes:** Display-completing only — no semantics change. The "Objects granted" / "Fields granted" header stat cards on Profile and PermSet detail still count true object-level and field-level grants respectively, so "granted" keeps its object-level meaning; FLS-only objects are additive in the tables and clearly labeled. `userEffectiveSignature` emits object (`O:`) tokens only for truthy CRUD flags, so the synthetic all-false rows added to `effectiveObjects()` produce no spurious diffs in Compare Users (§2.91) or Change Impact. No SOQL / data-pipeline change is required — the field data was already loaded; only the object-view assembly changed. The code comments, the §2.x version-history block in the JSX header, `package.json` / `index.html` version, and the Version History row below are part of the deferred release pass.

---

### 2.95 DATA-2 — Expand Query 1 (Profile) / Query 13 (System Permissions) to cover governance-relevant system permissions, including Query All Files — IMPLEMENTED in v3.8.0

> **Status:** Documented 2026-08-18, implemented same day. Follow-on to §2.72 BUG-16 / DATA-1, which only ever applied Decision 2(a) from that entry (add the three `TransferAny*` columns) and left Decision 2(b)/(c) — a broader curated list vs. the full documented set — unresolved. Manoj asked the app to surface which PermissionSets/Profiles hold "Query All Files," and investigation found `PermissionsQueryAllFiles` isn't in the enumerated column set at all. **Code, spec, and script changes are implemented and esbuild-verified.** The v3.8.0 build had a data bug in Query 1 — see §2.96 BUG-24, fixed in v3.8.1. Re-running `scripts/weekly-refresh.sh` and rebuilding the bundle to populate the new columns with real data is still pending, now blocked on the v3.8.1 fix landing first.

**Finding.** `SELECT FIELDS(ALL) FROM PermissionSet LIMIT 1` against the `bullhorn` org (API v67) confirms the object exposes **835** distinct `Permissions*` boolean columns. Query 1 (Profile) and Query 13 (`SystemPermissions_PermSet`) currently enumerate 15 combined (`PermissionsModifyAllData`, `PermissionsViewAllData`, `PermissionsManageUsers`, `PermissionsAuthorApex`, `PermissionsCustomizeApplication`, `PermissionsManageCustomPermissions`, `PermissionsApiEnabled`, `PermissionsBulkApiHardDelete`, `PermissionsDataExport`, `PermissionsViewSetup`, `PermissionsManageRoles`, `PermissionsRunReports`, `PermissionsTransferAnyEntity`, `PermissionsTransferAnyLead`, `PermissionsTransferAnyCase`). Verified in-org: 2 PermissionSets and 1 Profile currently have `PermissionsQueryAllFiles = true`.

Of the 820 uncaptured columns, the large majority are narrow feature-specific toggles (Tableau, Einstein/Agentforce, Slack, CDP, ITSM/industry-cloud features, etc.) with no bearing on core access governance. A curated pass grouped the governance-relevant remainder into 5 categories — **73 net-new fields** (i.e., excluding `PermissionsViewAllData`/`PermissionsModifyAllData`, which are already tracked).

**Decision (resolved 2026-08-18):** adopt the full 73-field shortlist below in one pass, rather than adding `QueryAllFiles` alone or a narrower subset. This mirrors Decision 2(c)'s intent from §2.72 without going all the way to "every documented field" (~835), which would sweep in hundreds of irrelevant feature toggles.

**Fields to add (73, grouped):**

1. **Data access / query-scope (13)** — same family as Query All Files: `PermissionsQueryAllFiles`, `PermissionsQueryNonVetoedFiles`, `PermissionsDeleteSalesforceFiles`, `PermissionsDownloadMaliciousFiles`, `PermissionsManageMaliciousFiles`, `PermissionsCanQueryForeignTables`, `PermissionsRecordVisibilityAPI`, `PermissionsViewAllForeignKeyNames`, `PermissionsViewAllCustomSettings`, `PermissionsViewAllFieldsGlobal`, `PermissionsManageFilesAndAttachments`, `PermissionsSelectFilesFromSalesforce`, `PermissionsViewContent`
2. **User / access administration (16)** — `PermissionsManageInternalUsers`, `PermissionsManageProfilesPermissionsets`, `PermissionsAssignPermissionSets`, `PermissionsResetPasswords`, `PermissionsFreezeUsers`, `PermissionsManageSharing`, `PermissionsManageIpAddresses`, `PermissionsManagePasswordPolicies`, `PermissionsManageLoginAccessPolicies`, `PermissionsPasswordNeverExpires`, `PermissionsViewAllUsers`, `PermissionsViewAllProfiles`, `PermissionsManageUnlistedGroups`, `PermissionsManageSessionPermissionSets`, `PermissionsDelegatedPortalUserAdmin`, `PermissionsApiUserOnly`
3. **Identity / session security (16)** — `PermissionsManageTwoFactor`, `PermissionsForceTwoFactor`, `PermissionsDelegatedTwoFactor`, `PermissionsTwoFactorApi`, `PermissionsSkipIdentityConfirmation`, `PermissionsIdentityEnabled`, `PermissionsIdentityConnect`, `PermissionsManageAuthProviders`, `PermissionsManageRemoteAccess`, `PermissionsAllowLightningLogin`, `PermissionsMonitorLoginHistory`, `PermissionsManageNetworks`, `PermissionsViewEventLogFiles`, `PermissionsUseAnyApiAuth`, `PermissionsClientSecretRotation`, `PermissionsViewClientSecret`
4. **Privacy / compliance / classification (12)** — `PermissionsViewUserPII`, `PermissionsPrivacyDataAccess`, `PermissionsModifyDataClassification`, `PermissionsConsentApiUpdate`, `PermissionsManageCustomerDataOptOut`, `PermissionsManageGlobalPrivacyCenterVO`, `PermissionsViewAllDataGovTags`, `PermissionsModifyAllDataGovTags`, `PermissionsViewAllDataGovPolicies`, `PermissionsModifyAllDataGovPolicies`, `PermissionsViewDataGovTab`, `PermissionsUseCompliantDataSharing`
5. **Metadata / packaging / deploy (16)** — `PermissionsModifyMetadata`, `PermissionsInstallPackaging`, `PermissionsPublishPackaging`, `PermissionsCreatePackaging`, `PermissionsDownloadPackageVersionZips`, `PermissionsManageSandboxes`, `PermissionsManageDevSandboxes`, `PermissionsManageNamedCredentials`, `PermissionsManageCertificates`, `PermissionsManageCertificatesExpiration`, `PermissionsViewSecurityCommandCenter`, `PermissionsViewHealthCheck`, `PermissionsManageHealthCheck`, `PermissionsViewHealthAssessments`, `PermissionsManageHealthAssessments`, `PermissionsCanApproveUninstalledApps`

**Changes required (three places, must stay in sync — this is the same drift pattern flagged in §2.72 Defect B):**

1. **`PermissionExplorer.jsx`** — add all 73 columns to the `SELECT` in `SOQL.profile` (~line 397) and `SOQL.sysPerms` (~line 428); add them to the `SYSPERM_COLS` array (~line 454) so they're first-class instead of falling through the `extraCols` auto-detect path (which still works today, per §2.72 Decision 9, but surfaces an "extra column" info banner instead of being silent).
2. **`requirements_v3_0.md` §5.1** — mirror the same SELECT expansion in the canonical Query 1 and Query 13 definitions.
3. **`scripts/weekly-refresh.sh`** — mirror the same SELECT expansion in the `Profile` and `SystemPermissions_PermSet` dataset-definition lines (~75, ~83). This is the actual production data source since the v3.0 architecture change (§2.65 ARCH-1) rearchitected the weekly Cowork task to shell out to this script rather than issuing SOQL in-chat.

No rendering changes are required: the CSV parser already auto-detects any `Permissions*` column not in `SYSPERM_COLS` (`extraCols`/`extraColsFromProfile`, ~lines 907–942) and `tokenToNoun()` already renders any `S:Permissions*` token generically by stripping the `Permissions` prefix. Adding the columns to `SYSPERM_COLS` explicitly is about giving them first-class status and removing the info-level "extra column" warning, not about making them visible — they'd already be visible either way, per the mechanism validated when `PermissionsTransferAny*` shipped in v3.0.1.

**Acceptance Criteria:**
- `Profile.csv` and `SystemPermissions_PermSet.csv`, once re-exported, include all 73 new columns alongside the existing 15.
- No "extra column loaded" info banner appears for any of the 73 fields after the `SYSPERM_COLS` update (they're recognized as first-class, not auto-detected).
- Any Profile or PermissionSet with one of the 73 permissions `true` shows it under System Permissions in that grantor's detail view, in Prescribe Access's system-permission picker, and in CSV exports.
- A stale bundle exported before this change (15 columns) still loads without error — the 73 new columns default to `false`/absent per the existing tolerant-parser behavior (§2.72 Decision 9(a), already validated).

**Out of scope:**
- The other ~747 `Permissions*` columns not in the 5 categories above (Tableau, Einstein/Agentforce, Slack, CDP, ITSM/industry-cloud, etc.) — no governance relevance identified.
- Friendly-name mapping (`PermissionsQueryAllFiles` → "Query All Files") — per §2.72 Decision 4(a), the app keeps API names as-is; unchanged here.
- Any bundle rebuild or weekly-refresh run — per standing instruction, `scripts/weekly-refresh.sh` and the `bundles/` folder are not touched without explicit go-ahead in addition to the code/spec change itself.

---

### 2.96 BUG-24 — DATA-2's Query 1 (Profile) SOQL included 3 fields invalid on Profile, breaking weekly refresh — IMPLEMENTED in v3.8.1

> **Status:** Fixed same day as discovered. The v3.8.0 weekly refresh ran automatically after §2.95 shipped and failed twice with `REFRESH FAILED (3): REST export failed for Profile`, exit code 3. The prior bundle was untouched (weekly-refresh.sh's export-then-swap design means a failed run never overwrites good data — confirmed working as designed).

**Root cause.** §2.95's 73-field shortlist was curated from `SELECT FIELDS(ALL) FROM PermissionSet` only, on the assumption that `Profile` carries an identical `Permissions*` field set (true for the 15 pre-existing columns and true for 70 of the 73 new ones). Three fields are the exception: `PermissionsInstallPackaging`, `PermissionsPublishPackaging`, `PermissionsCreatePackaging` are valid on `PermissionSet` but do not exist on `Profile` — Salesforce carries legacy AppExchange-era names there instead (`PermissionsInstallMultiforce`, `PermissionsPublishMultiforce`, `PermissionsCreateMultiforce`, confirmed present via `SELECT FIELDS(ALL) FROM Profile`). A SOQL `SELECT` with even one invalid field name is rejected in its entirety by Salesforce, so the whole Query 1 REST export failed — not just those 3 columns.

**Fix.** Dropped the 3 invalid fields from Query 1 (Profile) only. They remain in Query 13 (PermissionSet/`SystemPermissions_PermSet`), where they're valid. No legacy-name substitution added for Profile — the packaging/AppExchange-install permission is a narrow, legacy-adjacent case; smallest-reasonable-change is to simply not track it on Profile rather than introduce asymmetric field-name-per-object handling for one permission. Query 1's governance-relevant column count is 77 (not 80); Query 13 remains 88.

**Changes (implemented in v3.8.1):**
- `PermissionExplorer.jsx` `SOQL.profile` — 3 fields removed (73→70 net-new; 80→77 total).
- `requirements_v3_0.md` §5.1 Query 1 — mirrored.
- `scripts/weekly-refresh.sh` `Profile` dataset line — mirrored.
- `SOQL.sysPerms` / Query 13 / `SYSPERM_COLS` / `scripts/weekly-refresh.sh`'s `SystemPermissions_PermSet` line — **unchanged**, all 3 fields confirmed valid there.
- esbuild verified; `bash -n` verified. Header/`APP_VERSION`/package.json/index.html bumped to v3.8.1.

**Lesson for future column additions:** `Profile` and `PermissionSet` don't share an identical `Permissions*` field set. Before adding a new column to both queries, verify it exists on **both** objects individually (`SELECT FIELDS(ALL) FROM Profile LIMIT 1` and `SELECT FIELDS(ALL) FROM PermissionSet LIMIT 1`), not just one.

**Out of scope:** Adding `PermissionsInstallMultiforce`/`PermissionsPublishMultiforce`/`PermissionsCreateMultiforce` to Query 1 as Profile-side equivalents — not requested, and would need separate SYSPERM_COLS/label handling to avoid presenting as an unrelated permission from the PermissionSet-side names.
...
Updated in v3.0.1 to include `PermissionsTransferAnyEntity`, `PermissionsTransferAnyLead`, `PermissionsTransferAnyCase` so the Transfer Record permission and its legacy variants are present in the dataset and selectable in the Prescribe Access picker (see §2.72 BUG-16). Updated again in v3.8.0 (§2.95 DATA-2) to add `PermissionsQueryAllFiles` plus 72 other governance-relevant system permissions, bringing the column set to 80. **Corrected in v3.8.1 (§2.96 BUG-24):** 3 of those 72 — `PermissionsInstallPackaging`, `PermissionsPublishPackaging`, `PermissionsCreatePackaging` — don't exist on `Profile` (Salesforce uses legacy names `PermissionsInstallMultiforce`/`PermissionsPublishMultiforce`/`PermissionsCreateMultiforce` there instead) and broke the weekly refresh's Profile export. Dropped from this query; column count is 77, no longer 80. They remain valid and present in Query 13 below (`PermissionSet` does have the modern names). The full BUG-15 mandate ("every `Permissions*` boolean on Profile" — all ~835 in the target org) remains a partially-applied target; the remaining columns are narrow feature-specific toggles (Tableau, Einstein/Agentforce, Slack, CDP, ITSM, etc.) with no identified governance relevance and are out of scope per §2.95.
...
Updated in v3.0.1 to include `PermissionsTransferAnyEntity`, `PermissionsTransferAnyLead`, `PermissionsTransferAnyCase`. See §2.72 BUG-16. Updated again in v3.8 (§2.95 DATA-2) to add `PermissionsQueryAllFiles` plus 72 other governance-relevant system permissions, bringing the column set to 88 and matching Query 1's column set exactly. Additional `Permissions*` columns can be added incrementally; the parser tolerates new columns without code changes.

```sql
SELECT Id, Name, Label, PermissionsModifyAllData, PermissionsViewAllData, PermissionsManageUsers, PermissionsAuthorApex, PermissionsCustomizeApplication, PermissionsManageCustomPermissions, PermissionsApiEnabled, PermissionsBulkApiHardDelete, PermissionsDataExport, PermissionsViewSetup, PermissionsManageRoles, PermissionsRunReports, PermissionsTransferAnyEntity, PermissionsTransferAnyLead, PermissionsTransferAnyCase, PermissionsQueryAllFiles, PermissionsQueryNonVetoedFiles, PermissionsDeleteSalesforceFiles, PermissionsDownloadMaliciousFiles, PermissionsManageMaliciousFiles, PermissionsCanQueryForeignTables, PermissionsRecordVisibilityAPI, PermissionsViewAllForeignKeyNames, PermissionsViewAllCustomSettings, PermissionsViewAllFieldsGlobal, PermissionsManageFilesAndAttachments, PermissionsSelectFilesFromSalesforce, PermissionsViewContent, PermissionsManageInternalUsers, PermissionsManageProfilesPermissionsets, PermissionsAssignPermissionSets, PermissionsResetPasswords, PermissionsFreezeUsers, PermissionsManageSharing, PermissionsManageIpAddresses, PermissionsManagePasswordPolicies, PermissionsManageLoginAccessPolicies, PermissionsPasswordNeverExpires, PermissionsViewAllUsers, PermissionsViewAllProfiles, PermissionsManageUnlistedGroups, PermissionsManageSessionPermissionSets, PermissionsDelegatedPortalUserAdmin, PermissionsApiUserOnly, PermissionsManageTwoFactor, PermissionsForceTwoFactor, PermissionsDelegatedTwoFactor, PermissionsTwoFactorApi, PermissionsSkipIdentityConfirmation, PermissionsIdentityEnabled, PermissionsIdentityConnect, PermissionsManageAuthProviders, PermissionsManageRemoteAccess, PermissionsAllowLightningLogin, PermissionsMonitorLoginHistory, PermissionsManageNetworks, PermissionsViewEventLogFiles, PermissionsUseAnyApiAuth, PermissionsClientSecretRotation, PermissionsViewClientSecret, PermissionsViewUserPII, PermissionsPrivacyDataAccess, PermissionsModifyDataClassification, PermissionsConsentApiUpdate, PermissionsManageCustomerDataOptOut, PermissionsManageGlobalPrivacyCenterVO, PermissionsViewAllDataGovTags, PermissionsModifyAllDataGovTags, PermissionsViewAllDataGovPolicies, PermissionsModifyAllDataGovPolicies, PermissionsViewDataGovTab, PermissionsUseCompliantDataSharing, PermissionsModifyMetadata, PermissionsInstallPackaging, PermissionsPublishPackaging, PermissionsCreatePackaging, PermissionsDownloadPackageVersionZips, PermissionsManageSandboxes, PermissionsManageDevSandboxes, PermissionsManageNamedCredentials, PermissionsManageCertificates, PermissionsManageCertificatesExpiration, PermissionsViewSecurityCommandCenter, PermissionsViewHealthCheck, PermissionsManageHealthCheck, PermissionsViewHealthAssessments, PermissionsManageHealthAssessments, PermissionsCanApproveUninstalledApps FROM PermissionSet WHERE IsOwnedByProfile = false ORDER BY Label
```
...
| 2026-06-v3.7.1 | June 15 2026 | **Bug-fix patch — implemented (release pass pending).** **BUG-23** (§2.94). Objects carrying field-level security (`FieldPermissions`) but no `ObjectPermissions` row were dropped from every object-centric view (grantor Objects list/count, user Effective Objects, and the Object reverse-lookup Grantors table), because those views were built solely from the `ObjectPermissions` indexes. Reported against the Bullhorn Community Profile, where `OrderItem` has 3 field grants and 0 object grants on implicit PS `0PS0P000000b5cDWAQ`; org-wide 746 objects appear in FieldPermissions vs 1,179 in ObjectPermissions. Fix adds helper `fieldOnlyObjectRows()` and surfaces "field-only" objects (CRUD all-false, `FLS-only (n)` badge) in `ParentDetailSubSections`, `effectiveObjects()`, and `ObjectDetail`. Display-completing only — stat cards keep object-level "granted" semantics; `userEffectiveSignature` emits `O:` tokens only on truthy CRUD so Compare/Change-Impact diffs are unaffected; no SOQL change. esbuild verified. Code comments / JSX header / `package.json` / `index.html` version bump remain for the release pass. |
| 2026-06-v3.7.0 | June 10 2026 | **Feature — implemented.** **UX-66** (§2.93). All CSV exports now include raw Salesforce Id columns alongside resolved names. Central change in `ExportCSVButton`: auto-appends any `*Id`/`id` row keys (scanning first 50 rows) not already among visible columns. Row enrichments where Ids were missing: Impact Matrix `groupId`; delete-impact `userId`; subset extras `subsetId`/`supersetId`; Field detail grant tables `permSetId` + (for profile-owned implicit PS, via `profileByImplicitPsId`) `profileId`+`profileName`; Object detail Grantors `permSetId`/`profileId`; Duplicate PermSets `permSetAId`/`permSetBId`; Migration profile pairs `profileAId`/`profileBId`. Exports only — zero UI changes. esbuild verified; APP_VERSION/package.json/index.html → v3.7.0. |
| 2026-06-v3.6.1 | June 4 2026 | **Patch — implemented.** **BUG-8b** (§2.92). Profile-owned (implicit) PermSets were rendered by their implicit PS Id (e.g. `00e3g000000HjPAAA0`) in provenance views instead of the owning Profile name. Added inverse map `profileByImplicitPsId` + a shared `grantorLabel()` helper resolving such rows to `"<Profile> (profile)"`. Applied to Field detail (PermSet/Group/Muting grant tables), Object detail "Grantors" (name + Profile click-through), and `resolveSourceLabel` (every "Granted by" pill). Semantic system-perm tags, audit-trail tables, and permset list views were already correct / unaffected. esbuild verified; APP_VERSION/package.json → v3.6.1. |
| 2026-05-v3.6 | May 29 2026 | **Feature release — implemented.** **UX-65** (§2.91) Compare Users. New tab diffing two users' EFFECTIVE permissions side by side: `userEffectiveSignature()` + `diffUserSources()` helpers; `CompareUsersTab` with two typeaheads (persisted in AppState `compareUserA/B`), summary pills, assignment-source diff (only-A/shared/only-B), and an object/field/system token diff via three `PairedDiffPane`s (v3.2 filters + running list) with an "only differences" toggle and CSV export. Cross-links: Explorer user → Compare (preload A/B); Compare → Change Impact (set user filter). "Make A match B → simulate ↗" pushes B's missing PermSets into Simulation Mode via new batch `onSimulationMutateMany` and jumps to Change Impact for blast radius. Built on the same effective-permission engine as Change Impact, so results reconcile. esbuild verified; header/APP_VERSION/package.json/index.html → v3.6. |
| 2026-05-v3.5.2 | May 29 2026 | **Patch — implemented.** **FIX-2.** (a) Nav version badge in the header was a hardcoded literal `v3.1 · Salesforce` (line ~7065) — stale since v3.1 through every release since (the `appVersion` bumps only affect exported bundles, not the on-screen badge). Now derived from `APP_VERSION` via regex so it can't drift again. (b) v3.5.1's `autocomplete="off"` didn't stop Safari/iCloud Passwords on the "Target user" typeahead — WebKit ignores it on fields it classifies as usernames. Added `type="search"` to the `SuggestionTypeahead` input (strong non-credential signal Safari respects) and reworded both "Target user" placeholders from "Type a user name…" to "Start typing a name…". esbuild verified; header/APP_VERSION/package.json/index.html → v3.5.2. |
| 2026-05-v3.5.1 | May 29 2026 | **Patch — implemented.** **FIX-1.** Suppress browser/OS credential autofill on free-text inputs. Safari/iCloud Passwords was showing the "Enable Password AutoFill" key popup on the Prescribe-Access "Target user" typeahead because the "user name" placeholder reads as a login field. Added a shared `NO_AUTOFILL` attribute bundle (`autoComplete/autoCorrect/autoCapitalize="off"`, `spellCheck=false`, generic `name`, plus `data-lpignore` / `data-1p-ignore` / `data-form-type=other` hints for 1Password/LastPass/Dashlane) and spread it onto the 5 free-text inputs (`SearchInput`, `Typeahead`, `SuggestionTypeahead`, the object filter, the permset search). esbuild verified; header/APP_VERSION/package.json/index.html → v3.5.1. |
| 2026-05-v3.5 | May 29 2026 | **Performance — implemented.** **PERF-2** (§2.90). New `VirtualTable` component (spacer-row virtualization; fixed-height main rows + measured variable-height expandable detail rows; `tableLayout:fixed`+`colgroup` for stable columns; inline data-index zebra). Applied to the Scenario-A Impact Matrix table (main row + measured Lost/Covered detail) and the Prescribe-Access Before/After debug table. Both `slice(0,2000)` caps removed — full result set scrolls with constant DOM footprint; text columns ellipsize with title tooltips. Trade-off: Impact Matrix loses per-row hover highlight (fixed widths + inline zebra). Variable-height windowing unit-tested (pads sum to total; visible rows always within window); esbuild verified; no `slice(0,2000)` caps remain. header/APP_VERSION/package.json/index.html → v3.5. |
| 2026-05-v3.4 | May 29 2026 | **Performance — implemented.** **PERF-1** (§2.89). `precomputeImpactMatrix` converted to generator `precomputeImpactMatrixGen` (yields every 50 users); `ScenarioA` + Load-All Phase 1 run it via `asyncChunked()` so the UI/progress bar stay responsive instead of freezing 5–15s; `ScenarioA` gained a cancellation guard; sync `precomputeImpactMatrix()` kept as a `drainGen()` wrapper. New `VirtualList` component windows the `ExplorerTab` item picker (uniform row height per entry type, 34/48px), removing the `slice(0,2000)` cap and keeping a constant DOM footprint; windowing verified gap-free across full scroll. Deferred: the two `slice(0,2000)` table renders (sticky headers / expandable rows — riskier). esbuild verified; header/APP_VERSION/package.json/index.html → v3.4. |
| 2026-05-v3.3 | May 29 2026 | **Internal refactor — implemented (no behavior change).** **REFACTOR-1** (§2.88). Collapsed the three duplicated sync `precompute*` functions to delegate to their async generators via new `drainGen()` (−~90 lines, single source of truth). `subsetExtras()` gained an optional `sigMap` param so the Redundancy memo reuses signatures cached on `overlap.entries` instead of recomputing `grantorSignature` twice per subset pair. Added `cancelled`-flag cleanup to the lazy `setTimeout` precomputes in Redundancy/Mergable/Duplicate tabs (no more stale-state writes on tab switch). Added `setIsSubset()` short-circuit helper + single-pass intersection in the overlap-V26 loop (verified equivalent to old idioms over 200k random set pairs). Centralized `APP_VERSION` constant (was hardcoded ×3). Deleted obsolete `pe-filter-tracker.jsx`. Deferred: async-chunking `precomputeImpactMatrix`, list virtualization. esbuild compile verified; header/package.json/index.html bumped to v3.3. |
| 2026-05-v3.2 | May 29 2026 | **Feature release — implemented.** **UX-64** (§2.87) Interactive Paired-Diff Explorer. `PairedDiffPane` — the single shared component behind every paired diff (Compare pane, Reconcile, and `ConsolidatePairPane` for the Mergable + Duplicate tabs) — rewritten to embed: (1) the Has/Missing/Shared/Differs legend as toggle filter buttons (descriptions as tooltips); (2) an AND/OR "combine" mode toggle governing multi-filter logic (OR = status-set intersection, AND = selection ⊆ row status set); (3) a session-only "Running list" sidebar with per-row `+`/`✓` add, category badge via `tokenCategory` (Object/Field/System from token prefix), per-item `×` remove, and clear-list. Row status derived per pair via `rowStatusSet` (both → Shared; exclusive → Has/Missing). Removed the redundant standalone `<PairedDiffLegend>` preceding the two pane call sites; retained it at the two table-only sites. Verified with esbuild compile. JSX header, `appVersion` (×3), `package.json`, `index.html` bumped to v3.2. Running list is intentionally NOT persisted to IDB (session-only, per request). |
